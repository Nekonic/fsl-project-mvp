...
def _in(role, *argv, timeout=60):
    return run(role, list(argv), timeout=timeout)
...
def _status_from(role, url, node=False):
    if node:
        ...
        return _in(role, "/nodejs/bin/node", "-e", script, timeout=30).stdout.strip()
    return _in(
...
def test_the_wiki_keeps_its_own_record_of_what_was_read(stack_is_up):
    before = _in(WIKI, "wc", "-l", "/var/log/nginx/read.log").stdout.split()[0]
    _status_from(TARGET, f"http://wiki.internal{SECRET}", node=True)
    after = _in(WIKI, "wc", "-l", "/var/log/nginx/read.log").stdout.split()[0]
...
def taken_through_the_app(stack_is_up):
    ...
    import time
    time.sleep(4)
    return _in(WIKI, "tail", "-5", "/var/log/nginx/read.log").stdout
