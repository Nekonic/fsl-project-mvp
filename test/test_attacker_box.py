"""The red team is a person at a shell, not a list of buttons.

The case catalogue is a scripted baseline: the same payloads every run, so one
score can be compared with the last. It is not the red team. A red team runs
tools it chose against a target it is still working out, and the buttons
cannot be that because every one of them encodes a path only this target has.

So the box has to be a box: the tools an attacker reaches for, and a way to
attribute what is done with them. These check both halves, because a shell
with nothing installed is a demo and a shell nothing scores is a toy.
"""
...
# Recon, discovery, scanning, exploitation, and the plumbing to improvise
# with. Not an arbitrary list: each is what a person reaches for at a stage,
# and the catalogue cannot reach any of them.
TOOLS = [
    "nmap", "whatweb",                 # what is there
    "ffuf", "gobuster",                # what is served
    "nikto", "sqlmap",                 # what is wrong with it
    "hydra",                           # what it will let you in with
    "curl", "wget", "nc", "dig", "jq",  # everything else
]
...
def test_a_directory_brute_force_has_a_wordlist_to_use(stack_is_up):
    # ffuf without a list is ffuf that does nothing, and the list Kali's
    # `wordlists` package advertises is a symlink into a package that has to
    # be installed separately. It was not, and the path in the shell's own
    # help text pointed at nothing.
...
def test_the_shell_is_told_what_to_attack(stack_is_up):
    # Nobody should have to read compose.yaml to find the target.
...
def test_the_console_reports_both_addresses_the_box_can_leave_by(box):
    # HTTP goes through the stamping proxy, raw TCP does not. A window scored
    # against the wrong one matches no alert at all, which reads as a defence
    # that missed.
...
# An attacker dials a company, not the appliance in front of it. The shell
# used to be told to attack `waf-edge`, which says both that there is a WAF
# and what it is called - and no console anywhere shows that, because no
# attacker would ever know it.
...
def test_choosing_an_origin_moves_the_source_without_renaming_the_site(stack_is_up, box):
    """The console picks where the shell's traffic leaves from.

    Otherwise the only way to attack from elsewhere is to type an internal
    name, which is the thing this removed.
    """
    elsewhere = next(
